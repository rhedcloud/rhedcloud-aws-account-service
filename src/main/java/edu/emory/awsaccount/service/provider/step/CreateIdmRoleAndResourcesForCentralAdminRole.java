...
public class CreateIdmRoleAndResourcesForCentralAdminRole extends AbstractStep implements Step {
    ...
    protected List<Property> run() throws StepException {
        ...
        if (allocatedNewAccount && (newAccountId != null && !newAccountId.equals(PROPERTY_VALUE_NOT_AVAILABLE))) {
            ...
            String centralAdminRoleGuid = getStepPropertyValue("CREATE_LDS_GROUP_FOR_CENTRAL_ADMIN_ROLE", "guid");
            ...
            try {
                // Main fields
                String roleNameTemplate = "RGR_AWS-ACCOUNT_NUMBER-RHEDcloudCentralAdministratorRole";
                ...
                String ent1dn = "CN=RHEDcloudCentralAdministratorRole,OU=ACCOUNT_NUMBER,OU=AWS,DC=emory,DC=edu";
                ent1.setEntitlementDN(ent1dn.replace("ACCOUNT_NUMBER", newAccountId));
                ent1.setEntitlementGuid(centralAdminRoleGuid);
                ...
                res2.setResourceDescription("Provisions members to group RHEDcloudCentralAdministratorRole on MS LDS Healthcare Connector");
                res2.setResourceCategoryKey("group");
                Entitlement ent2 = res2.newEntitlement();
                String ent2dn = "CN=RHEDcloudCentralAdministratorRole,OU=ACCOUNT_NUMBER,OU=AWS,DC=emory,DC=edu";
                ent2.setEntitlementDN(ent2dn.replace("ACCOUNT_NUMBER", newAccountId));
                ent2.setEntitlementGuid(centralAdminRoleGuid);
