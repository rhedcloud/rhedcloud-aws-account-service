...
public class CreateRsAccountCfnStack extends AbstractStep implements Step {
    ...
    public void init (String provisioningId, Properties props,
        ...
        String cloudTrailSuffix = getProperties().getProperty("cloudTrailSuffix", null);
        setCloudTrailSuffix(cloudTrailSuffix);
        logger.info(LOGTAG + "cloudTrailSuffix is: " + getCloudTrailSuffix());
        ...
        String region = getProperties().getProperty("region", "us-east-1");
        setRegion(region);
        logger.info(LOGTAG + "region is: " + getRegion());

        // rhedCloudIdp is the name of the identity provider.
        String rhedCloudIdp = getProperties().getProperty("rhedCloudIdp", null);
        ...
        String rhedCloudSamlIssuer = getProperties().getProperty("rhedCloudSamlIssuer", null);
        ...
        String rhedCloudSecurityRiskDetectionServiceUserArn = getProperties().getProperty("rhedCloudSecurityRiskDetectionServiceUserArn", null);
        setRhedCloudSecurityRiskDetectionServiceUserArn(rhedCloudSecurityRiskDetectionServiceUserArn);
        logger.info(LOGTAG + "rhedCloudSecurityRiskDetectionServiceUserArn is: " + getRhedCloudSecurityRiskDetectionServiceUserArn());

        // rhedCloudAwsAccountServiceUserArn is the ARN that the
        // SecurityRiskDetectionService uses to access linked accounts.
        String rhedCloudAwsAccountServiceUserArn = getProperties().getProperty("rhedCloudAwsAccountServiceUserArn", null);
        setRhedCloudAwsAccountServiceUserArn(rhedCloudAwsAccountServiceUserArn);
        logger.info(LOGTAG + "rhedCloudAwsAccountServiceUserArn is: " + getRhedCloudAwsAccountServiceUserArn());

        // rhedCloudMaintenanceOperatorRoleArn...I have no idea what this is
        // but I am guessing it is some role for the ops team.
        String rhedCloudMaintenanceOperatorRoleArn = getProperties().getProperty("rhedCloudMaintenanceOperatorRoleArn", null);
        setRhedCloudMaintenanceOperatorRoleArn(rhedCloudMaintenanceOperatorRoleArn);
        logger.info(LOGTAG + "rhedCloudMaintenanceOperatorRoleArn is: " + getRhedCloudMaintenanceOperatorRoleArn());
    ...
    protected List<Property> run() throws StepException {
        ...
        if (allocatedNewAccount && (newAccountId != null && !newAccountId.equals(PROPERTY_VALUE_NOT_AVAILABLE))) {
            logger.info(LOGTAG + "allocateNewAccount is true and newAccountId is " +
                newAccountId + ". Sending a Stack.Generate-Request to create the rhedcloud-aws-rs-account stack in a new account.");
            ...
            ProvisioningStep step3 = getProvisioningStepByType("DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE");
            String accountSequenceNumber = null;
            if (step3 != null) {
                logger.info(LOGTAG + "Step DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE found.");
                accountSequenceNumber = getResultProperty(step3, "accountSequenceNumber");
                logger.info(LOGTAG + "Property accountSequenceNumber from preceding step is: " + newAccountId);
                addResultProperty("accountSequenceNumber", accountSequenceNumber);
            }
            else {
                String errMsg = "Step DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE not found. Cannot determine account sequence number.";
            ...
            logger.info(LOGTAG + "Getting properties from preceding steps...");
            ProvisioningStep step4 = getProvisioningStepByType("VERIFY_NEW_ACCOUNT_ADMIN_DISTRO_LIST");
            ...
            if (step4 != null) {
                ...
                logger.info(LOGTAG + "Property accountAlias from preceding step is: " + accountAlias);
            ...
            try {
                ...
                addResultProperty(parameter1.getKey(), parameter1.getValue());
                ...
                addResultProperty(parameter2.getKey(), parameter2.getValue());
                ...
                addResultProperty(parameter3.getKey(), parameter3.getValue());
                ...
                addResultProperty(parameter4.getKey(), parameter4.getValue());
                ...
                addResultProperty(parameter6.getKey(), parameter6.getValue());
                ...
                addResultProperty(parameter7.getKey(), parameter7.getValue());
                ...
                // Log out all capabilities and add them to the step properties.
            ...
            if (results.size() == 1) {
                ...
                logger.info(LOGTAG + "Stack result is: " + stackResult.getStackStatus());
        ...
        // If allocateNewAccount is false, log it and add result props.
            ...
            logger.info(LOGTAG + "allocateNewAccount is false. no need to create the rhedcloud-aws-rs-account stack.");
            addResultProperty("allocateNewAccount", "false");
        }

        // Update the step.
        if (allocatedNewAccount == false || stackCreated == true) {
    ...
    protected List<Property> simulate() throws StepException {
        long startTime = System.currentTimeMillis();
        String LOGTAG = getStepTag() + "[CreateRsAccountCfnStack.simulate] ";
    ...
    protected List<Property> fail() throws StepException {
        long startTime = System.currentTimeMillis();
        String LOGTAG = getStepTag() + "[CreateRsAccountCfnStack.fail] ";
    ...
    private void setCloudTrailSuffix (String suffix) throws StepException {
        if (suffix == null) {
            String errMsg = "cloudTrailSuffix property is null. Can't continue.";
            throw new StepException(errMsg);
        }
    ...
    private String getCloudTrailName(String accountAlias) {
        return accountAlias + "-" + getCloudTrailSuffix();
    }
    ...
    private String getAddHipaaIamPolicy() {
        ...
        if (req.getComplianceClass().equalsIgnoreCase(HIPAA_COMPLIANCE_CLASS)) {
    ...
    private void setRhedCloudIdp (String idp) throws StepException {
        if (idp == null) {
            String errMsg = "rhedCloudIdp property is null. Can't continue.";
            throw new StepException(errMsg);
        }
        m_rhedCloudIdp = idp;
    }
    ...
    private void setRhedCloudSamlIssuer (String issuer) throws StepException {
        if (issuer == null) {
            String errMsg = "rhedCloudSamlIssuer property is null. Can't continue.";
            throw new StepException(errMsg);
        }
    ...
    private void setRhedCloudSecurityRiskDetectionServiceUserArn (String arn) throws StepException {
        if (arn == null) {
            String errMsg = "setRhedCloudSecurityRiskDetectionServiceUserArn property is null. Can't continue.";
            throw new StepException(errMsg);
        }
    ...
    private void setRhedCloudAwsAccountServiceUserArn (String arn) throws StepException {
        if (arn == null) {
            String errMsg = "setRhedCloudAwsAccountServiceUserArn property is null. Can't continue.";
            throw new StepException(errMsg);
        }
    ...
    private void setRhedCloudMaintenanceOperatorRoleArn (String arn) throws StepException {
        if (arn == null) {
            String errMsg = "rhedCloudMaintenanceOperatorRoleArn property is null. Can't continue.";
            throw new StepException(errMsg);
        }
    ...
    private void setRegion (String region) throws StepException {
        if (region == null) {
    ...
    private String getCloudFormationTemplateBody() throws StepException {
        String LOGTAG = getStepTag() + "[CreateRsAccountCfnStack.getCloudFormationTemplateBody] ";
        String templateBody;
