...
public class GenerateNewAccount extends AbstractStep implements Step {
    ...
    protected List<Property> run() throws StepException {
        ...
        boolean allocateNewAccount;
        if (step_DETERMINE_NEW_OR_EXISTING_ACCOUNT != null) {
            logger.info(LOGTAG + "Step DETERMINE_NEW_OR_EXISTING_ACCOUNT found.");
            String sAllocateNewAccount = getResultProperty(step_DETERMINE_NEW_OR_EXISTING_ACCOUNT, "allocateNewAccount");
            ...
            String errMsg = "Step DETERMINE_NEW_OR_EXISTING_ACCOUNT not found. Can't continue.";
        ...
        logger.info(LOGTAG + "Getting properties from preceding steps...");
        ProvisioningStep step_DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE = getProvisioningStepByType("DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE");
        String accountSequenceNumber;
        if (step_DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE != null) {
            logger.info(LOGTAG + "Step DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE found.");
            accountSequenceNumber = getResultProperty(step_DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE, "accountSequenceNumber");
            ...
            String errMsg = "Step DETERMINE_NEW_ACCOUNT_SEQUENCE_VALUE not found. Can't continue.";
            logger.error(LOGTAG + errMsg);
            throw new StepException(errMsg);
        }
        ...
        if (allocateNewAccount == true) {
            ...
            if (step3 != null) {
                ...
                logger.info(LOGTAG + "Property accountEmailAddress from preceding step is: " + accountEmailAddress);
            }
            else {
            ...
            // Build account name
            String newAccountName = getAccountSeriesName() + " " + accountSequenceNumber;
            ...
            try {
                ...
                String errMsg = "An error occurred creating the account. The exception is: " + e.getMessage();
                logger.error(LOGTAG + errMsg);
            ...
            while (createComplete == false) {
                ...
                casResult = getAwsOrganizationsClient().describeCreateAccountStatus(casRequest);
                ...
                if (state.equals(IN_PROGRESS) == false) {
                    ...
                    try {
                        ...
                        String errMsg = "An error occurred waiting for AWS to create the new account. The exception is: " + ie.getMessage();
                        logger.error(LOGTAG + errMsg);
        ...
        // If allocateNewAccount and accountSequenceNumber is false, log it and add result props.
        else {
            logger.info(LOGTAG + "allocateNewAccount is false. no need to create a new account.");
            addResultProperty("allocatedNewAccount", Boolean.toString(allocatedNewAccount));
            addResultProperty("newAccountId", PROPERTY_VALUE_NOT_APPLICABLE);
        }
