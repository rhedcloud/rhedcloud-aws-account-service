...
/**
 * This step propagates the VPC route to the TGW route tables.
 */
public class PropagateVpcCidrToTgwRouteTable extends AbstractStep implements Step {
    ...
    protected List<Property> run() throws StepException {
        ...
        if (step_DETERMINE_VPC_CONNECTION_METHOD == null) {
            String errMsg = "Step DETERMINE_VPC_CONNECTION_METHOD not found. Can't continue.";
            logger.error(LOGTAG + errMsg);
            throw new StepException(errMsg);
        }

        boolean createVpc = Boolean.parseBoolean(getStepPropertyValue("DETERMINE_VPC_TYPE", "createVpc"));
        String vpcConnectionMethod = getResultProperty(step_DETERMINE_VPC_CONNECTION_METHOD, "vpcConnectionMethod");

        if (createVpc && vpcConnectionMethod.equals("TGW")) {
            String transitGatewayAccountId = getResultProperty(step_DETERMINE_VPC_CONNECTION_METHOD, "transitGatewayAccountId");
            ...
            try {
                ...
                EnableTransitGatewayRouteTablePropagationRequest enableTransitGatewayRouteTablePropagationRequest
                        = new EnableTransitGatewayRouteTablePropagationRequest()
                                .withTransitGatewayAttachmentId(transitGatewayAttachmentId);

                // enable each propagation route table id discovered by DETERMINE_VPC_CONNECTION_METHOD
                int i = 0;
                do {
                    String key = "transitGatewayPropagationRouteTableId_0_" + i;
                    String transitGatewayPropagationRouteTableId = getResultProperty(step_DETERMINE_VPC_CONNECTION_METHOD, key);
                    if (transitGatewayPropagationRouteTableId == null)
                        break;

                    enableTransitGatewayRouteTablePropagationRequest.setTransitGatewayRouteTableId(transitGatewayPropagationRouteTableId);
                    EnableTransitGatewayRouteTablePropagationResult enableTransitGatewayRouteTablePropagationResult
                            = ec2Client.enableTransitGatewayRouteTablePropagation(enableTransitGatewayRouteTablePropagationRequest);

                    addResultProperty("transitGatewayPropagationState_" + i, enableTransitGatewayRouteTablePropagationResult.getPropagation().getState());
            ...
            logger.info(LOGTAG + "Bypass TGW route table propagation: not creating VPC or not TGW connectivity");
        }
