...
public class ListVpcIds extends AbstractStep implements Step {
	...
	protected List<Property> run() throws StepException {
		...
		AccountDeprovisioningRequisition req = getAccountDeprovisioning()
		...
		List<VirtualPrivateCloud> results = null;
		try {
			long queryStartTime = System.currentTimeMillis();
			results = vpc.query(querySpec, rs);
			long queryTime = System.currentTimeMillis() - queryStartTime;
			int vpcCount = results.size();
			logger.info(LOGTAG + "Queried for VPC metadata in " + queryTime +
					" ms. Found " + vpcCount + " VPC(s).");
		...
		// If there are results, log them and add them to the result
		// properties.
		if (results != null && results.size() > 0) {
			List<String> vpcIds = new ArrayList<String>();
			ListIterator<VirtualPrivateCloud> li = results.listIterator();
			while (li.hasNext()) {
				VirtualPrivateCloud vpcResult = li.next();
